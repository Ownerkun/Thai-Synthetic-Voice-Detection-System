# API Mockup — ตัวอย่าง Request/Response จริง (สำหรับ Frontend)

เอกสารนี้แยกไว้ให้ frontend เริ่มเขียนโค้ดได้โดยไม่ต้องรอ backend/Docker รันอยู่
ทุก JSON ในนี้ **จับมาจาก request จริงที่ยิงผ่านโค้ด backend จริง** (ไม่ได้พิมพ์เดามือ)
ตอนที่ยังไม่มีไฟล์โมเดล .onnx ระบบจึงใช้ `MockSpoofAnalyzer` — **ตัวเลขคะแนน/verdict เป็นค่าจำลอง**
แต่ **โครงสร้าง JSON (field/type) เป็นของจริง 100%** และจะไม่เปลี่ยนตอนเปลี่ยนมาใช้โมเดลจริง
