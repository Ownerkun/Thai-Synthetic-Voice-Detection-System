...
> ไม่ใช่การออกแบบลอย ๆ ชื่อคลาส เมธอด และเส้นทางข้อมูลตรงกับไฟล์ในโปรเจกต์
> ส่วนที่ยังไม่ได้พัฒนาจะระบุกำกับไว้ชัดเจนว่า *(อยู่นอกขอบเขต / ยังไม่พัฒนา)*
...
```mermaid
flowchart LR
  user(["ผู้ใช้งานทั่วไป<br/>End User"])
  ...
  class user,admin,researcher actor
  class uc1,uc2,uc3,uc4,uc5,uc6,uc7,uc8 uc
  class uc9,uc10,uc11,uc12,uc13,uc14 admincase
```

**หมายเหตุการออกแบบ**
- UC1–UC8 **ไม่ต้อง login** ตามหลัก Data Minimization (พ.ร.บ. คุ้มครองข้อมูลส่วนบุคคล) เพราะไม่มีความสามารถใดที่ต้องผูกกับตัวตนผู้ใช้
- UC9–UC11 **ต้อง login** เพราะกระทบระบบส่วนรวม (RBAC แบบชั้นเดียว)
...
> ตรงกับแนวคิด "แทนที่ front-end" ของ Tak et al. (2022) พอดี
...
flowchart TB
  ...
  class choose,validate,lenChk,decide,fb,partial dec
...
> **จุดสำคัญ** — Research Layer กับ Backend Layer **ไม่แชร์ dependency กัน**
...
flowchart TB
  ...
  parity -- "ไม่ตรง" --> stop["หยุด — ห้าม deploy"]
...
> เพื่อสื่อสารทิศทาง ไม่ใช่สิ่งที่มีอยู่แล้ว — ควรระบุให้ชัดตอนนำเสนอ
> เพื่อไม่ให้เข้าใจผิดว่าระบบเสร็จสมบูรณ์แล้ว
